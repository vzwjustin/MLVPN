...
sudo stdbuf -oL -eL env MLVPN_QUIC_INSECURE=1 "$MLVPN" --debug -v -c "$WORKDIR/server.conf" -u mlvpn \
...
sudo stdbuf -oL -eL env MLVPN_QUIC_INSECURE=1 "$MLVPN" --debug -v -c "$WORKDIR/client.conf" -u mlvpn \
